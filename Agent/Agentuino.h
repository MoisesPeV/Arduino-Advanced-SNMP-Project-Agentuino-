...
typedef struct SNMP_VALUE {
    ...
    SNMP_ERR_CODES decode1(int32_t *value) {
       ...
       SNMP_ERR_CODES decode2(int32_t *value) {
        if (syntax == SNMP_SYNTAX_INT32) {
            //Serial.println("LLEGA AQUI");
     ...
     SNMP_ERR_CODES decode3(int32_t *value) {
        if (syntax == SNMP_SYNTAX_INT32) {
            //Serial.println("LLEGA AQUI");
            //tiempoMuestreo1 = data[0];
            *value = data[0];
   ...
   SNMP_ERR_CODES decodeIP(byte *value) {
    if (syntax == SNMP_SYNTAX_IP_ADDRESS || syntax == SNMP_SYNTAX_NSAPADDR) {
        if (size != 4) {
            // El tamaño debe ser exactamente 4 bytes para una dirección IP
            clear();
            return SNMP_ERR_WRONG_LENGTH;
        }

        memcpy(value, data, 4); // Copiar los 4 bytes de la dirección IP
        clear();
        return SNMP_ERR_NO_ERROR;
    } else {
        clear();
